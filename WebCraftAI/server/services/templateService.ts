...
/**
 * Collection of pre-designed website templates that can be filled with AI-generated content
 * This approach significantly reduces token usage compared to generating entire websites from scratch
 */
...
export interface WebsiteTemplate {
  ...
  name: string;
  description: string;
  html: string; // Template HTML with placeholders for content
  css: string;  // Template CSS 
  structure: WebsiteTemplateStructure; // Structure of content needed to fill the template
}

export interface WebsiteTemplateStructure {
...
export interface TemplateSectionType {
  id: string;
  type: string;
  fields: string[]; // Fields to be filled with AI-generated content
}
...
/**
 * Fills a template with content
 */
export function fillTemplate(template: WebsiteTemplate, content: WebsiteContent): { html: string; css: string } {
  ...
  // For each section in the template structure, find the corresponding content and fill it
  template.structure.sections.forEach(section => {
    const sectionData = content.sections.find(s => s.type === section.type);
    
    if (sectionData) {
      // Fill each field specified in the section
      section.fields.forEach(field => {
        const value = sectionData[field as keyof typeof sectionData];
        if (value !== undefined) {
          // Replace all instances of the placeholder with the content
  ...
  if (content.colorScheme) {
    ...
    if (colorClass === 'blue' || colorClass === 'green' || colorClass === 'red' || 
        colorClass === 'yellow' || colorClass === 'purple') {
  ...
  if (content.fontStyle) {
    const fontFamily = content.fontStyle === 'modern' ? "'Poppins', sans-serif" : 
                       content.fontStyle === 'classic' ? "'Merriweather', serif" : 
                       content.fontStyle === 'minimal' ? "'Inter', sans-serif" : 
                       content.fontStyle === 'playful' ? "'Comic Neue', cursive" : 
                       "'Roboto', sans-serif";
